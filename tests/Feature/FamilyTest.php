...
class FamilyTest extends TestCase
{
    ...
    public function test_a_family_is_named_after_the_last_name(): void
    {
        $this->assertSame('The Krona family', Team::familyNameFor('Eric  Krona'));
        $this->assertSame('The Krona family', Team::familyNameFor('Anna Maria Krona'));
        $this->assertSame('The Cher family', Team::familyNameFor('Cher'));
        $this->assertSame('Familjen Krona', Team::familyNameFor('Eric Krona', 'sv'));
    }

    public function test_existing_users_get_a_family_of_their_own_when_migrating(): void
    {
        ...
        (require database_path('migrations/2026_09_25_000050_create_families_for_existing_users.php'))->up();

        $this->assertSame('Familjen Krona', $eric->fresh()->currentTeam->name);
        $this->assertSame('The Doe family', $jane->fresh()->currentTeam->name);
        $this->assertTrue($eric->fresh()->currentTeam->personal_team);
        $this->assertTrue($eric->fresh()->ownsTeam($eric->fresh()->currentTeam));
        $this->assertSame(1, $alreadyHasOne->ownedTeams()->count());
    }

    public function test_creating_a_family_twice_keeps_the_first(): void
    {
        $user = User::factory()->create(['name' => 'Eric Krona']);

        $first = app(CreatePersonalFamily::class)->handle($user);
        $second = app(CreatePersonalFamily::class)->handle($user->fresh());

        $this->assertTrue($first->is($second));
        $this->assertSame(1, Team::count());
    }

    public function test_registering_in_the_panel_creates_a_family(): void
    {
        ...
        $this->assertSame('The Krona family', $user->currentTeam->name);

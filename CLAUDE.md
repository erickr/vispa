...
A family is a Jetstream team (`App\Models\Team`; code and tables keep Jetstream's "team" names because Jetstream hardcodes them, but everything a user sees says "family"). Jetstream is used for its **backend only**: models, `HasTeams`, `TeamPolicy` and the actions in `app/Actions/Jetstream`. All of its UI is gone. The family page is a Filament page, `App\Filament\Pages\Family` (`/app/family`, in the sidebar), which calls Jetstream's actions for inviting, removing, renaming, creating, switching, leaving and deleting. **Filament owns authentication**: Fortify's routes are off (`Fortify::ignoreRoutes()`), and Jetstream's own URIs (`/teams/*`, `/user/profile`, `/dashboard`, `/login`) redirect into the panel or 404 (`/team-invitations/*`), see `routes/web.php`. Sanctum, API tokens, 2FA, profile photos and roles are left out. With no roles, every member is equal and only the owner manages membership.
...
**Recipes belong to a family** (`recipes.team_id`) and every member can open and edit every recipe in it. `Recipe::scopeAccessibleTo($user)` is the one access rule: recipes in any of the user's families, plus their own (so leaving a family keeps what you wrote). Both recipe resources and the fork pickers scope through it; use it for any new recipe query rather than filtering on `owner_user_id`. A new recipe gets the owner's current family from a `creating` hook. **Ingredients** a user owns are shared with everyone they share a family with (`User::familyMemberIds()`, used by `Ingredient::visibleTo()`, `IngredientPolicy` and the unique-name rule). The ingredient picker's "keep the current pick resolvable" exception must come from the saved row, never from submitted state. `FamilyIsolationTest` pins all of this down.

Invitations are sent as `App\Mail\FamilyInvitation` (template `resources/views/mail/family-invitation.blade.php`, linking new people to the panel's registration) and accepted at `/families/invitations/{id}` by `AcceptFamilyInvitation`: only from the invited address, switching the member into the family, and confirming with a Filament notification.

Do not rerun `php artisan jetstream:install`. It overwrites `User.php`, the factory, the seeder, `vite.config.js` and `app.css`, downgrades Tailwind to v3, strips `dark:` classes from Filament views, and brings back the UI removed here.
